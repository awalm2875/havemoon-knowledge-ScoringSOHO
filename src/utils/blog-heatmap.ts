...
export function getLatestPosts(posts: BlogPost[], count = 1): LatestPost[] {
  return [...posts]
    .sort(
      (a, b) =>
        (b.data.updatedDate ?? b.data.date).valueOf() -
        (a.data.updatedDate ?? a.data.date).valueOf(),
    )
...
function getPostsByDate(posts: BlogPost[]) {
  ...
  const addActivity = (date: Date, post: BlogPost, event: HeatmapPost['event']) => {
  ...
  for (const post of posts) {
    addActivity(post.data.date, post, 'published');

    if (post.data.updatedDate && getDateKey(post.data.updatedDate) !== getDateKey(post.data.date)) {
      addActivity(post.data.updatedDate, post, 'updated');
    }
  }
